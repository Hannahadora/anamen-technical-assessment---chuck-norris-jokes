  ...
  <div>
    <div class="search-tooltip">
      <div class="search-box">
        ...
        />
        <span>
          <img src="../assets/images/scope_black.png" alt="search_icn" />
        </span>
      </div>
      <div class="tooltip-dropdown" v-if="showDropdown">
        ...
        <div
          ...
          :key="idx"
          @click="goToJoke(category)"
        >
          <div><img src="../assets/images/flash_icon.png" alt="flash" /></div>
          <h2 class="title">
            {{ truncateSentence(category) || "uncategorized" }}
          </h2>
        </div>
      </div>
    </div>
...
const goToJoke = (category) => {
  showDropdown.value = false;
  search.value = truncateSentence(category)
  let xy = category.split("-");
  const el = jokes.value.find((joke => joke.id.includes(xy[xy.length - 1])));
  router.push(`/joke/1/${el.id}`);
};
...
onMounted(async () => {
  ...
  allAvailableCategories.value = jokes.value?.flatMap(
    (el) => el.categories + "-" + el.value + "-" + el.id
  );
});
</script>
